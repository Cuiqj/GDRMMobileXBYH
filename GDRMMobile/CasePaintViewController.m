...
//    CGRect * frame = self.IconType.frame;
//    CGRect * frame = self.IconModelName.frame;
...
- (IBAction)chooseWidth:(id)sender {
    if ([self.modelname isEqualToString:@"出口匝道A"] || [self.modelname isEqualToString:@"入口匝道A"] || [self.modelname isEqualToString:@"出口匝道B"]) {
        return;
    }
...
//            NSData *data = UIImageJPEGRepresentation(mapImage, 0.6);
    ...
    //保存数据到casephoto
    [self setcasePhotowithMap];
}

- (void)setcasePhotowithMap{
    CaseMap * caseMap = [CaseMap caseMapForCase:self.caseID];
    self.photo = [CasePhoto casePhotoById:self.caseID];
    if (caseMap && !self.photo) {
        self.photo = [CasePhoto newDataObjectWithEntityName:@"CasePhoto"];
        self.photo.myid = self.caseID;
        self.photo.proveinfo_id = caseMap.myid;
        self.photo.photo_name = @"勘验草图";
        [[AppDelegate App] saveContext];
    }
}
